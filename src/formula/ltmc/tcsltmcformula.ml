...
let make_uniquely_bound f =
  ...
  | FNeg f' -> let (v', f'') = binder vars subst f' in
                         (v', FNeg f'')
  | FNext f' -> let (v', f'') = binder vars subst f' in
                         (v', FNext f'')
  | f -> (vars, f)
  in
...
let rec eval_metaformula f =
  ...
  | Tcsmetaformula.FUnaryOp ("()", f) -> FNext (eval_metaformula f)
  ...
  | _ -> failwith("Formula is not able to be interpreted as LTMC");;
...
let rec subst_formula f sub tar =
  if f = sub then tar
         ...
         | FNext f' -> FNext (subst_formula f' sub tar)
...
let guarded_transform f =
  ...
  | FNext g -> FNext (guarded_transform' g false)
  | FMu (v, g) ->
...
let normal_form_formula_to_decomposed_formula f =
	...
	|	FOr (f1, f2) -> let (j1, pr1) = process_formula f1 in
						...
						(j, max pr1 pr2)
	|	FNext f -> let (j1, pr1) = process_formula f in
						   let j = process_decomp_formula (FIntNext j1) in
						   (j, pr1)
