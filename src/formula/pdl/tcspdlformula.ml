...
let rec eval_metaformula f =
  ...
  | Tcsmetaformula.FLabelledOp ("<>", p, f) -> FDiamond (eval_program p, eval_metaformula f)
  | Tcsmetaformula.FLabelledOp ("[]", p, f) -> FBox (eval_program p, eval_metaformula f)
  | _ -> failwith("Formula is not able to be interpreted as PDL")
and eval_program p =
  match p with
    Tcsmetaformula.FProp s -> FAtom (identifier_to_str s)
  | Tcsmetaformula.FBinaryOp (".", f1, f2) -> FConcat (eval_program f1, eval_program f2)
  | Tcsmetaformula.FOr (f1, f2) -> FChoice (eval_program f1, eval_program f2)
  | Tcsmetaformula.FUnaryOp ("*", g) -> FStar (eval_program g)
  | Tcsmetaformula.FUnaryOp ("?", g) -> FQuestion (eval_metaformula g)
